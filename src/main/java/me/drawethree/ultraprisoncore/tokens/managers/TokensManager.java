...
import me.drawethree.ultraprisoncore.api.events.BananaPrisonBlockBreakEvent;
import me.drawethree.ultraprisoncore.api.events.player.BananaPrisonPlayerTokensReceiveEvent;
...
@SuppressWarnings({"deprecation", "ConstantConditions", "ConditionCoveredByFurtherCondition", "StringBufferReplaceableByString", "UnusedAssignment"})
public class TokensManager {


    private final BananaPrisonTokens plugin;
    ...
    private final HashMap<UUID, Double> multipliers = new HashMap<>();
    ...
    public TokensManager(BananaPrisonTokens plugin) {
        this.plugin = plugin;
    ...
    public void giveTokens(OfflinePlayer p, long amount, CommandSender executor, ReceiveCause cause) {
        Schedulers.async().run(() -> {
            ...
            if(multipliers.containsKey(p.getUniqueId())){
                initialAmount *= multipliers.get(p.getUniqueId());
            }

            BananaPrisonPlayerTokensReceiveEvent event = new BananaPrisonPlayerTokensReceiveEvent(cause, p, initialAmount);
            ...
            if (!p.isOnline()) {
                ...
                if (executor != null && executor instanceof ConsoleCommandSender) {
                    p.getPlayer().sendMessage(plugin.getMessage("tokens_received_console")
                            .replace("%tokens%", String.format("%,d", finalAmount))
                            .replace("%player%", executor == null ? "Console" : executor.getName()));
                }
            }
    ...
    public void withdrawTokens(Player executor, long amount, int value) {
        Schedulers.async().run(() -> {
            ...
            if (!notFit.isEmpty()) {
                notFit.forEach(itemStack -> this.giveTokens(executor, amount * item.getAmount(), null, ReceiveCause.REDEEM));
            }
    ...
    public void addBlocksBroken(CommandSender sender, Player player, long amount) {
        ...
        BananaPrisonBlockBreakEvent event = new BananaPrisonBlockBreakEvent(player, amount);
     ...
     * @param multiplier The multiplier that will be applied
     * @author _LightDream
     */
