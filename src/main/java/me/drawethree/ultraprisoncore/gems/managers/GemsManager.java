...
import me.drawethree.ultraprisoncore.api.events.player.BananaPrisonPlayerGemsReceiveEvent;
...
public class GemsManager {
    ...
    private final BananaPrisonGems plugin;
    ...
    public GemsManager(BananaPrisonGems plugin) {
        this.plugin = plugin;
        this.reload();
    ...
    public static ItemStack createGemsItem(long amount, int value) {
        return ItemStackBuilder.of(Material.DOUBLE_PLANT)
                .amount(value).name("&e&l" + String.format("%,d", amount) + " GEMS")
                .lore("&7Right-Click to Redeem")
    ...
    private void addIntoTable(Player player) {
        Schedulers.async().run(() -> this.plugin.getCore().getPluginDatabase().addIntoGems(player));
    }
    ...
    public void giveGems(OfflinePlayer p, long amount, CommandSender executor, ReceiveCause cause) {
        ...
        BananaPrisonPlayerGemsReceiveEvent event = new BananaPrisonPlayerGemsReceiveEvent(cause, p, amount);
    ...
    public void withdrawGems(Player executor, long amount, int value) {
        Schedulers.async().run(() -> {
            ...
            if (!notFit.isEmpty()) {
                notFit.forEach(itemStack -> this.giveGems(executor, amount * item.getAmount(), null, ReceiveCause.REDEEM));
            }
