...
public class Gang {
    ...
    public boolean leavePlayer(Player p) {
        ...
        this.getOnlinePlayers().forEach(player -> player.sendMessage(BananaPrisonGangs.getInstance().getMessage("gang-player-left").replace("%player%", p.getName())));
        p.sendMessage(BananaPrisonGangs.getInstance().getMessage("gang-left").replace("%gang%", this.name));
        return true;
    }
    ...
    public boolean joinPlayer(Player p) {
        ...
        this.getOnlinePlayers().forEach(player -> player.sendMessage(BananaPrisonGangs.getInstance().getMessage("gang-player-joined").replace("%player%", p.getName())));
        this.gangMembers.add(p.getUniqueId());
        p.sendMessage(BananaPrisonGangs.getInstance().getMessage("gang-joined").replace("%gang%", this.name));
        return true;
    }
