...
import me.drawethree.ultraprisoncore.api.events.BananaPrisonAutoSellEvent;
import me.drawethree.ultraprisoncore.api.events.BananaPrisonSellAllEvent;
import me.drawethree.ultraprisoncore.autosell.api.BananaPrisonAutoSellAPI;
import me.drawethree.ultraprisoncore.autosell.api.BananaPrisonAutoSellAPIImpl;
...
public final class BananaPrisonAutoSell implements BananaPrisonModule {
	...
	@Getter
	private BananaPrisonAutoSellAPI api;
	...
	public BananaPrisonAutoSell(BananaPrisonCore UltraPrisonCore) {
	...
	public void enable() {
		...
		this.api = new BananaPrisonAutoSellAPIImpl(this);
	}
					...
					if (!enabledAutoSell.contains(e.getPlayer().getUniqueId())) {
						...
						for (IWrappedRegion reg : regions) {
							if (regionsAutoSell.containsKey(reg.getId()) && regionsAutoSell.get(reg.getId()).sellsMaterial(e.getBlock().getType())) {
								...
								BananaPrisonAutoSellEvent event = new BananaPrisonAutoSellEvent(e.getPlayer(), region, e.getBlock(), amount);
					...
					if (c.args().size() == 0 || c.args().size() == 1) {
						...
						for (IWrappedRegion region : regions) {
							if (regionsAutoSell.containsKey(region.getId())) {
								...
								if (core.isModuleEnabled(BananaPrisonMultipliers.MODULE_NAME)) {
								...
								//Call post autosell
								BananaPrisonSellAllEvent event = new BananaPrisonSellAllEvent(c.sender(), autoSellRegion);
								Events.call(event);
								break;
							}
						}
