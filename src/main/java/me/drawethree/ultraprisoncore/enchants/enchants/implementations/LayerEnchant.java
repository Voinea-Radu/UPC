...
public class LayerEnchant extends BananaPrisonEnchantment {

    private double chance;


    public LayerEnchant(BananaPrisonEnchants instance) {
        super(instance, 10);
    }
    ...
    public void onBlockBreak(BlockBreakEvent e, int enchantLevel) {
        ...
        if (chance * enchantLevel >= ThreadLocalRandom.current().nextDouble(100)) {
            ...
            if (regions.size() > 0) {
                ...
                double total = plugin.getCore().getMultipliers().getApi().getTotalToDeposit(p, totalDeposit);
